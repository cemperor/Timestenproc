 ...
 static int hash(const hash_t *tptr, const unsigned int key) {
...
//      i=(i<<3)+(*key++ - '0');
 ...
 VMDEXTERNSTATIC engineStruct* hash_lookup(const hash_t *tptr, const unsigned int key) {
 ...
 VMDEXTERNSTATIC int hash_insert(hash_t *tptr,  const unsigned int key, engineStruct data) {
 ...
 //  node->data=data;
   ...
   memcpy(&node->data, &data, sizeof(engineStruct));
 ...
 VMDEXTERNSTATIC int hash_delete(hash_t *tptr, const unsigned int key) {
...
//     if (!strcmp(node->key, key))
 ...
 VMDEXTERNSTATIC char * hash_stats(hash_t *tptr) {
   ...
   sprintf(buf, "%u slots, %u entries, aEnd %1.2f ALOS",
